...
{
    if (bus_type < IO_BUS_MAX && (meson_reg_map[bus_type]) && (meson_reg_max[bus_type] >= reg)) {
...
int aml_regmap_update_bits(u32 bus_type, unsigned int reg, unsigned int mask, unsigned int val)
{
    int ret = 0;
...
int aml_read_cbus(unsigned int reg)
{
    ...
    ret = aml_reg_read(IO_CBUS_BASE, reg << 2L, &val);
    if (ret) {
...
void aml_write_cbus(unsigned int reg, unsigned int val)
{
    ...
    ret = aml_reg_write(IO_CBUS_BASE, reg << 2L, val);
    if (ret) {
...
void aml_cbus_update_bits(unsigned int reg, unsigned int mask, unsigned int val)
{
    int ret;

    ret = aml_regmap_update_bits(IO_CBUS_BASE, reg << 2L, mask, val);
    if (ret) {
        pr_err("write cbus reg %x error %d\n", reg, ret);
    }
}
...
void aml_aobus_update_bits(unsigned int reg, unsigned int mask, unsigned int val)
{
    int ret;
...
{
    int ret, val;
    ret = aml_reg_read(IO_VAPB_BUS_BASE, reg << 2L, &val);
    if (ret) {
...
void aml_write_vcbus(unsigned int reg, unsigned int val)
{
    ...
    ret = aml_reg_write(IO_VAPB_BUS_BASE, reg << 2L, val);
    if (ret) {
...
void aml_vcbus_update_bits(unsigned int reg, unsigned int mask, unsigned int val)
{
    int ret;

    ret = aml_regmap_update_bits(IO_VAPB_BUS_BASE, reg << 2L, mask, val);
    if (ret) {
        pr_err("write vcbus reg %x error %d\n", reg, ret);
    }
}
...
int aml_read_dosbus(unsigned int reg)
{
    ...
    ret = aml_reg_read(IO_APB_BUS_BASE, reg << 2L, &val);
    if (ret) {
        pr_err("read vcbus reg %x error %d\n", reg, ret);
        return -1;
    } else {
...
void aml_write_dosbus(unsigned int reg, unsigned int val)
{
    ...
    ret = aml_reg_write(IO_APB_BUS_BASE, reg << 2L, val);
    if (ret) {
        pr_err("write vcbus reg %x error %d\n", reg, ret);
    }
}
...
void aml_dosbus_update_bits(unsigned int reg, unsigned int mask, unsigned int val)
{
    int ret;

    ret = aml_regmap_update_bits(IO_APB_BUS_BASE, reg << 2L, mask, val);
    if (ret) {
        pr_err("write vcbus reg %x error %d\n", reg, ret);
    }
}
...
int aml_read_hiubus(unsigned int reg)
{
    ...
    ret = aml_reg_read(IO_HIUBUS_BASE, reg << 2L, &val);
    if (ret) {
        pr_err("read cbus reg %x error %d\n", reg, ret);
        return -1;
    } else {
...
void aml_write_hiubus(unsigned int reg, unsigned int val)
{
    ...
    ret = aml_reg_write(IO_HIUBUS_BASE, reg << 2L, val);
    if (ret) {
        pr_err("write cbus reg %x error %d\n", reg, ret);
    }
}
...
void aml_hiubus_update_bits(unsigned int reg, unsigned int mask, unsigned int val)
{
    int ret;

    ret = aml_regmap_update_bits(IO_HIUBUS_BASE, reg << 2L, mask, val);
    if (ret) {
        pr_err("write cbus reg %x error %d\n", reg, ret);
    }
}

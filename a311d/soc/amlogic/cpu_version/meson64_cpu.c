...
int __init meson_cpu_version_init(void)
{
    ...
    meson_cpu_version[MESON_CPU_VERSION_LVL_MAJOR] = readl(assist_hw_rev) >> 24L;

    ver = (readl(assist_hw_rev) >> 8L) & 0xff;

    meson_cpu_version[MESON_CPU_VERSION_LVL_MINOR] = ver;
    ver = (readl(assist_hw_rev) >> 16L) & 0xff;
    ...
    pr_info("Meson chip version = Rev%X (%X:%X - %X:%X)\n", meson_cpu_version[MESON_CPU_VERSION_LVL_MINOR],
            meson_cpu_version[MESON_CPU_VERSION_LVL_MAJOR], meson_cpu_version[MESON_CPU_VERSION_LVL_MINOR],
            meson_cpu_version[MESON_CPU_VERSION_LVL_PACK], meson_cpu_version[MESON_CPU_VERSION_LVL_MISC]);
    return 0;
}

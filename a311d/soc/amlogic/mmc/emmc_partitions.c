...
struct aml_dtb_rsv {
    u8 data[DTB_BLK_SIZE * DTB_BLK_CNT - 4L * sizeof(unsigned int)];
    unsigned int magic;
...
int mmc_write_internal(struct mmc_card *card, unsigned int dev_addr, unsigned int blocks, void *buf)
{
...
static int _dtb_write(struct mmc_card *mmc, int blk, unsigned char *buf)
{
    ...
    do {
        ret = mmc_write_internal(mmc, blk_tmp, MAX_TRANS_BLK, src);
        if (ret) {
...
static int _dtb_init(struct mmc_card *mmc)
{
    ...
    while (cpy >= 0) {
        blk = ((get_reserve_partition_off_from_tbl() + DTB_RESERVE_OFFSET) >> bit) + cpy * DTB_BLK_CNT;
...
int amlmmc_dtb_write(struct mmc_card *mmc, unsigned char *buf, int len)
{
    ...
    if (valid == 0) {
        ...
        if (info->stamp[0] != info->stamp[1]) {
            ...
            dtb->timestamp = 1 + (stamp_after(info->stamp[1], info->stamp[0]) ? info->stamp[1] : info->stamp[0]);
        } else {
...
ssize_t mmc_dtb_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
...
ssize_t mmc_dtb_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    unsigned char *dtb_ptr = NULL;
...
static void mmc_prepare_mrq(struct mmc_card *card, struct mmc_request *mrq, struct scatterlist *sg, unsigned int sg_len,
                            unsigned int dev_addr, unsigned int blocks, unsigned int blksz, int write)
{
...
unsigned int mmc_capacity(struct mmc_card *card)
{
    if (!mmc_card_sd(card) && mmc_card_is_blockaddr(card)) {
        ...
        return card->csd.capacity << (card->csd.read_blkbits - 9L);
    }
}
...
static int mmc_transfer(struct mmc_card *card, unsigned int dev_addr, unsigned int blocks, void *buf, int write)
{
    ...
    cur_part_number = card->ext_csd.part_config & EXT_CSD_PART_CONFIG_ACC_MASK;
    if (cur_part_number != user_part_number) {
        switch_partition = true;
        original_part_config = card->ext_csd.part_config;
        cur_part_number = original_part_config & (~EXT_CSD_PART_CONFIG_ACC_MASK);
        ret = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_PART_CONFIG, cur_part_number, card->ext_csd.part_time);
        if (ret) {
            return ret;
        }
    ...
    mmc_prepare_mrq(card, &mrq, &sg, 1, dev_addr, blocks, 1 << card->csd.read_blkbits, write);

    mmc_wait_for_req(card->host, &mrq);

    ret = mmc_check_result(&mrq);
    if (switch_partition) {
        ret = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_PART_CONFIG, original_part_config,
...
static int amlmmc_write_tuning_para(struct mmc_card *card, unsigned int dev_addr)
{
    unsigned int size;
...
static int mmc_read_partition_tbl(struct mmc_card *card, struct mmc_partitions_fmt *read_pt_fmt)
{
    int ret = 0, start_blk, size, blk_cnt;
    int bit = card->csd.read_blkbits;
    ...
    memset(read_pt_fmt, 0, sizeof(struct mmc_partitions_fmt));
    ...
    if ((strncmp(read_pt_fmt->magic, MMC_PARTITIONS_MAGIC, sizeof(read_pt_fmt->magic)) == 0) &&
        read_pt_fmt->part_num > 0 && read_pt_fmt->part_num <= MAX_MMC_PART_NUM &&
        read_pt_fmt->checksum == mmc_partition_tbl_checksum_calc(read_pt_fmt->partitions, read_pt_fmt->part_num)) {
        ret = 0; /* everything is OK now */
    } else {
        if (strncmp(read_pt_fmt->magic, MMC_PARTITIONS_MAGIC, sizeof(read_pt_fmt->magic)) != 0) {
            pr_info("magic error: %s\n", read_pt_fmt->magic);
        } else if ((read_pt_fmt->part_num < 0) || (read_pt_fmt->part_num > MAX_MMC_PART_NUM)) {
            ...
            pr_info("checksum error: pt_fmt->checksum=%d,calc_result=%d\n", read_pt_fmt->checksum,
                    mmc_partition_tbl_checksum_calc(read_pt_fmt->partitions, read_pt_fmt->part_num));
        }
...
static struct hd_struct *add_emmc_each_part(struct gendisk *disk, int partno, sector_t start, sector_t len, int flags,
                                            char *pname)
{
    struct hd_struct *p;
    ...
    p->info = pinfo;
    (void)sprintf(p->info->volname, "%s", pname);
...
static int add_emmc_partition(struct gendisk *disk, struct mmc_partitions_fmt *add_pt_fmt)
{
    unsigned int i;
    ...
    for (i = 0; i < add_pt_fmt->part_num; i++) {
        pp = &(add_pt_fmt->partitions[i]);
        offset = pp->offset >> 9L; /* unit:512 bytes */
        size = pp->size >> 9L;     /* unit:512 bytes */
        if ((offset + size) <= cap) {
            ...
            pr_info("[%sp%02d] %20s  offset 0x%012llx, size 0x%012llx %s\n", disk->disk_name, 1 + i, pp->name,
                    offset << 9L, size << 9L, IS_ERR(ret) ? "add fail" : "");
        } else {
    ...
    proc_card = proc_create("inand", 0444L, NULL, &card_proc_fops);
    if (!proc_card) {
        pr_info("[%s] create /proc/inand fail.\n", __func__);
    }

    /* create /proc/ntd */
    if (!proc_create("ntd", 0444L, NULL, &card_proc_fops)) {
...
static ssize_t emmc_part_table_get(struct class *class, struct class_attribute *attr, char *buf)
{
    ...
    part_table = kmalloc_array(MAX_MMC_PART_NUM, sizeof(struct partitions), GFP_KERNEL);
    if (!part_table) {
...
static ssize_t get_bootloader_offset(struct class *class, struct class_attribute *attr, char *buf)
{
    int offset = 0;
...
static struct class_attribute aml_version = __ATTR(version, 0444, emmc_version_get, NULL);
static struct class_attribute aml_part_table = __ATTR(part_table, 0444, emmc_part_table_get, NULL);
static struct class_attribute aml_store_device = __ATTR(store_device, 0444, store_device_flag_get, NULL);
static struct class_attribute bootloader_offset = __ATTR(bl_off_bytes, 0444, get_bootloader_offset, NULL);
...
int add_fake_boot_partition(struct gendisk *disk, char *name, int idx)
{
    ...
    idx_tmp ^= 1;
    (void)sprintf(fake_name, name, idx_tmp);
...
int aml_emmc_partition_ops(struct mmc_card *card, struct gendisk *disk)
{
    ...
    buffer = kmalloc(512L, GFP_KERNEL);
    if (!buffer) {
        return -ENOMEM;
    }

...
int32_t HdiVideoComposition::VideoModuleInit(void)
{
    ...
    InitFunc func = (int32_t(*)(LayerFuncs **funcs))(dlsym(videoModule_, VO_LAYER_FUNC_INIT));
    if (func == nullptr) {
...
uint32_t HdiVideoComposition::CreateVideoLayerForHdiLayer(HdiLayer &hdiLayer)
{
    ...
    DISPLAY_CHK_RETURN(((ret != DISPLAY_SUCCESS) || (voLayerId == 0xffffffff)), 0xffffffff,
...
int32_t HdiVideoComposition::SetLayers(std::vector<HdiLayer*> &layers, HdiLayer &clientLayer)
{
    ...
    for (auto layer : matchedLayers) {
        if ((GetVoLayerId(*layer) != 0xffffffff) ||
            (CreateVideoLayerForHdiLayer(*layer) != 0xffffffff)) {

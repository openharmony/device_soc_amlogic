...
void DrmConnector::InitModes(drmModeConnector c)
{
    ...
    for (int i = 0; i < c.count_modes; i++) {
        ...
        if ((mPreferenceId == -1) && (mode->type & DRM_MODE_TYPE_PREFERRED)) {
...
{
    if ((mType == DISP_INTF_HDMI) && (mPropBrightnessId == 0xFFFFFFFF)) {
...
int32_t DrmConnector::SetBrightness(uint32_t level)
{
    ...
    brFd = open("/sys/class/brightness/brightness", O_RDWR);
    if (brFd < 0) {
    ...
    *num = static_cast<int32_t>(mModes.size());
    ...
    if (modes != nullptr) {
        for (const auto &modeMap : mModes) {
            ...
            mode.ConvertToHdiMode(*(modes + (*num - (i + 1))));
            i++;
        }
    }
    return DISPLAY_SUCCESS;
...
DrmModeBlock::~DrmModeBlock()
{
    ...
    if ((mBlockId != 0xFFFFFFFF) && (drmFd >= 0)) {
